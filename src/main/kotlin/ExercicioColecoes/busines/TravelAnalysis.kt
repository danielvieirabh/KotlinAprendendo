package ExercicioColecoes.busines


import ExercicioColecoes.data.DataStorage.Companion.generateShips
import ExercicioColecoes.entity.Ship
import ExercicioColecoes.entity.Travel
import ExercicioColecoes.utils.ShipType
...
class TravelAnalysis private constructor() {
    ...
    companion object {
        ...
        fun getAverageCostForBlackPearl(ships: List<Ship>, travels: List<Travel>): Double {
            ...
            return travels.filter { it.ship == blackPearl }.map { it.cost }.average()
        }

        // Qual o nome do navio com a maior capacidade de carga?
        fun getShipWithBiggestCargoCapacity(ships: List<Ship>): String {
            return ships.maxByOrNull { it.cargoCapacity }?.name.toString() //maxByorNull retorna o elemento
        }
        ...
        fun getShipNameForLongestTravelAndDuration(travels: List<Travel>): Map<String, Int> {
           val duracao = travels.maxByOrNull { it.durationDays }
            return mapOf(Pair(duracao?.ship?.name ?: "", duracao?.durationDays ?: 0))
        }
